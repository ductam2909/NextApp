...
router.get('/', async (req, res) => {
  try {
    ...
    if (search !== '') {
      const tours = await Tours.find({ name: { $regex: search } }).sort({ createdAt: 1 }).skip(skip).limit(limit)
      const count = await Tours.find({ name: { $regex: search } }).sort({ createdAt: 1 }).skip(skip).limit(limit).count()
